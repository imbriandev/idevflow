...
This is a living acceptance ledger. `Pending` means the behavior has not yet been implemented and proven in iDevFlow.
...
| Packaging | Local, Git, and npm registry installation | package root | Local/Git loading, package smoke validation, npm installation/migration, and release-process documentation implemented |
...
| Package identity | Current namespace, provenance, release record, and manual-boundary confirmation | package root, `docs/` | TypeScript-only package gate, validation evidence, and annotated Git tags implemented |
