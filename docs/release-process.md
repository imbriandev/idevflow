...
For an iOS candidate, `idev_release promote` changes only the local base branch after a candidate-bound approval. A founder may then approve one exact internal-beta action that archives, exports, and uploads that candidate through the credential-isolated Apple bridge; it never selects testers or distributes. A macOS handoff records distribution readiness without signing, archive, upload, notarization, or distribution. See the [Apple release capability matrix](release-capabilities.md).
