...
function workerPrompt(packet: Awaited<ReturnType<typeof readWorkerPacket>>): string {
  return `You are an isolated Canopy pipeline worker. The deterministic packet below is your complete authority.\n\n${JSON.stringify(packet, null, 2)}\n\nRequired procedure:\n1. Call canopy_preflight with stage=build, write=true, the exact task/risk/claims from the packet.\n2. Call canopy_context with the packet stage/risk/task before loading specialist references; read only selected references relevant to the claimed surface. Inspect only the writer worktree and implement only this slice.\n3. Review acceptance, correctness, privacy, accessibility, and regressions while the writer session is still active. If repair is needed, call canopy_pipeline_worker repair before each repair cycle; stop when denied.\n4. Run canopy_verify at least at the packet verification profile after the final repair; use matrix=true when the packet names multiple platforms.\n5. Run canopy_session postflight and finish. Do not modify source afterward.\n6. Run a fresh integration verification on the ready commit.\n7. Submit the source-bound machine-readable passing verdict with canopy_pipeline_worker submit.\n\nNever integrate, promote, push, upload, distribute, alter the work graph, expand claims, or treat prose as a receipt. If blocked, call canopy_pipeline_worker block with concise evidence.`;
}
...
export class PiWorkerLauncher implements WorkerLauncher {
  ...
  async launch(input: WorkerLaunchInput): Promise<SupervisedProcessResult> {
    ...
    const args = [
      ...
      "--tools", "read,write,edit,canopy_context,canopy_preflight,canopy_session,canopy_verify,canopy_exec,canopy_pipeline_worker",
    ];
    ...
    return runSupervised({
      ...
      environment: {
        ...credentials,
        CANOPY_WORKER_PACKET: input.packetPath,
        CANOPY_WORKER_PACKET_DIGEST: input.packetDigest,
        CANOPY_WORKER_CAPABILITY: input.capability,
        CANOPY_WORKER_EXTENSION: input.extensionPath,
