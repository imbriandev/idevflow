...
async function writeStageReceipt(repository: RepositoryDescriptor, receipt: StageReceipt): Promise<void> {
  await mkdir(join(repository.primaryRoot, ".canopy", "receipts", "stages"), { recursive: true, mode: 0o700 });
  await writeFileAtomically(join(repository.primaryRoot, ".canopy", "receipts", "stages", `${receipt.stage}-${receipt.sourceCommit}.json`), `${JSON.stringify(receipt, null, 2)}\n`);
}
...
async function completedSlices(repository: RepositoryDescriptor, graphFingerprint: string): Promise<Set<string>> {
  const directory = join(repository.primaryRoot, ".canopy", "receipts", "stages");
  let names: string[];
...
async function readApproval(repository: RepositoryDescriptor): Promise<PlanApproval> {
  try { return JSON.parse(await readFile(join(repository.primaryRoot, ".canopy", "approvals", "plan.json"), "utf8")) as PlanApproval; }
...
export async function approvePlan(repository: RepositoryDescriptor, approvedBy: string): Promise<PlanApproval> {
  ...
  await mkdir(join(repository.primaryRoot, ".canopy", "approvals"), { recursive: true, mode: 0o700 });
  await writeFileAtomically(join(repository.primaryRoot, ".canopy", "approvals", "plan.json"), `${JSON.stringify(approval, null, 2)}\n`);
