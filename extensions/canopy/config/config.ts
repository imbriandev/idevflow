import { access, copyFile, lstat, mkdir, readFile, rename } from "node:fs/promises";
...
export const DEFAULT_CONFIG: CanopyConfig = {
  ...
  documents: {
    productMemory: "docs/canopy/product-memory.json",
    slcSpec: "docs/canopy/slc.json",
    workGraph: "docs/canopy/work-graph.json",
    privacyReview: "docs/canopy/privacy-review.json",
    monetization: "docs/canopy/monetization.json",
    releaseManifest: "docs/canopy/release.json",
  },
...
export async function migrateLegacyRuntimeDirectory(primaryRoot: string): Promise<void> {
  const legacy = join(primaryRoot, ".pi-ios");
  const current = join(primaryRoot, ".canopy");
  let legacyStat;
  try { legacyStat = await lstat(legacy); }
  catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return; throw error; }
  if (legacyStat.isSymbolicLink()) throw new SafetyKernelError("Refusing to migrate a symbolic-link legacy runtime directory");
  try { await lstat(current); throw new SafetyKernelError("Both .pi-ios and .canopy exist; remove neither and reconcile them manually"); }
  catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
  await rename(legacy, current);
}

export function configPath(primaryRoot: string): string {
  return join(primaryRoot, ".canopy", "config.json");
}

function optionalString(value: unknown, name: string): string | undefined {
  if (value === undefined) return undefined;
  if (typeof value !== "string" || !value.trim()) throw new SafetyKernelError(`Canopy config ${name} must be a non-empty string when set`);
  return value;
}

function positiveInteger(value: unknown, name: string, minimum = 1): number {
  if (!Number.isInteger(value) || (value as number) < minimum) {
    throw new SafetyKernelError(`Canopy config ${name} must be an integer of at least ${minimum}`);
  }
  return value as number;
}

export function validateConfig(value: unknown): CanopyConfig {
  if (!value || typeof value !== "object") throw new SafetyKernelError("Canopy config must be a JSON object");
  const config = value as Partial<CanopyConfig>;
  if (config.schemaVersion !== CONFIG_SCHEMA_VERSION) {
    throw new SafetyKernelError(`Unsupported Canopy config schema: ${String(config.schemaVersion)}`);
  }
  ...
  if (!config.xcode || typeof config.xcode !== "object") throw new SafetyKernelError("Canopy config xcode must be an object");
  if (config.xcode.platform !== "ios" && config.xcode.platform !== "macos") throw new SafetyKernelError("Canopy config xcode.platform must be ios or macos");
  if (!Array.isArray(config.xcode.requiredPlatforms) || config.xcode.requiredPlatforms.length === 0 || config.xcode.requiredPlatforms.some((platform) => platform !== "ios" && platform !== "macos") || new Set(config.xcode.requiredPlatforms).size !== config.xcode.requiredPlatforms.length || !config.xcode.requiredPlatforms.includes(config.xcode.platform)) throw new SafetyKernelError("Canopy config xcode.requiredPlatforms must contain unique ios/macos values including xcode.platform");
  ...
  if (!config.simulator || typeof config.simulator !== "object") throw new SafetyKernelError("Canopy config simulator must be an object");
  ...
  if (typeof config.simulator.keepBooted !== "boolean") throw new SafetyKernelError("Canopy config simulator.keepBooted must be boolean");
  if (simulatorLeaseSeconds < verificationTimeoutSeconds * 2 + 300) {
    throw new SafetyKernelError("Canopy simulator lease must cover two verification actions plus a 300-second safety margin");
  }

  if (!config.verification || typeof config.verification !== "object") throw new SafetyKernelError("Canopy config verification must be an object");
  ...
  if (typeof config.verification.requireXcresult !== "boolean") throw new SafetyKernelError("Canopy config verification.requireXcresult must be boolean");
  if (!Array.isArray(config.verification.requiredScreenshotVariants) || config.verification.requiredScreenshotVariants.length === 0 || config.verification.requiredScreenshotVariants.some((variant) => typeof variant !== "string" || !variant)) {
    throw new SafetyKernelError("Canopy config verification.requiredScreenshotVariants must be a non-empty string array");
  }

  if (!config.quality || typeof config.quality !== "object") throw new SafetyKernelError("Canopy config quality must be an object");
  if (typeof config.quality.requireXCTestEvidence !== "boolean") throw new SafetyKernelError("Canopy config quality.requireXCTestEvidence must be boolean");
  if (!config.quality.performanceBudgets || typeof config.quality.performanceBudgets !== "object" || Array.isArray(config.quality.performanceBudgets)) throw new SafetyKernelError("Canopy config quality.performanceBudgets must be an object");
  for (const [metric, budget] of Object.entries(config.quality.performanceBudgets)) {
    if (!metric.trim() || !Number.isFinite(budget) || budget <= 0) throw new SafetyKernelError("Canopy config quality.performanceBudgets requires non-empty metric names and positive finite budgets");
  }

  if (!config.documents || typeof config.documents !== "object") throw new SafetyKernelError("Canopy config documents must be an object");
  for (const key of ["productMemory", "slcSpec", "workGraph", "privacyReview", "monetization", "releaseManifest"] as const) {
    ...
    if (path.startsWith("/") || path.split(/[\\/]/).includes("..")) throw new SafetyKernelError(`Canopy config documents.${key} must stay project-relative`);
  }
  if (!config.release || typeof config.release !== "object") throw new SafetyKernelError("Canopy config release must be an object");
  ...
  if (config.release.defaultTarget !== "testflight-internal" && config.release.defaultTarget !== "testflight-external") {
    throw new SafetyKernelError("Canopy config release.defaultTarget is invalid");
  }
  if (!config.pipeline || typeof config.pipeline !== "object") throw new SafetyKernelError("Canopy config pipeline must be an object");
  if (typeof config.pipeline.enabled !== "boolean") throw new SafetyKernelError("Canopy config pipeline.enabled must be boolean");
  ...
  return config as CanopyConfig;
}

export async function loadConfig(primaryRoot: string): Promise<CanopyConfig> {
  await migrateLegacyRuntimeDirectory(primaryRoot);
...
export interface ConfigMigrationPlan {
  ...
  readonly config?: CanopyConfig;
}

function migrateLegacy(raw: Record<string, unknown>): CanopyConfig {
...
export async function applyConfigMigration(primaryRoot: string): Promise<CanopyConfig> {
...
export async function initializeConfig(primaryRoot: string): Promise<CanopyConfig> {
  await migrateLegacyRuntimeDirectory(primaryRoot);
  const path = configPath(primaryRoot);
  await mkdir(join(primaryRoot, ".canopy"), { recursive: true, mode: 0o700 });
  try {
    await access(path);
