...
const STATUS_KEY = "canopy";

export function updateStatus(ctx: ExtensionContext, state: SessionState): void {
  if (!state.stage) {
    ctx.ui.setStatus(STATUS_KEY, ctx.ui.theme.fg("dim", "Canopy · idle"));
    return;
  }
  ctx.ui.setStatus(STATUS_KEY, ctx.ui.theme.fg("accent", `Canopy · ${state.stage}`));
}

export function formatDashboard(state: SessionState): string {
  if (!state.stage) return "Canopy is idle.";
...
export function updateCoordinatorStatus(ctx: ExtensionContext, snapshot: CoordinatorSnapshot): void {
  ...
  ctx.ui.setStatus(STATUS_KEY, ctx.ui.theme.fg("accent", `Canopy · ${lifecycle} → ${snapshot.route}`));
}
