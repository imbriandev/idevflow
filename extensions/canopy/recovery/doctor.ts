...
export async function diagnoseSessions(repository: RepositoryDescriptor): Promise<SessionDiagnostic[]> {
  ...
  for (const worktree of await registeredWorktrees(repository)) {
    if (worktree.branch?.startsWith("refs/heads/canopy/") && !registeredPaths.has(worktree.path)) {
      diagnostics.push({ sessionId: `orphan:${worktree.path}`, severity: "warning", message: `Unregistered Canopy worktree on ${worktree.branch}`, recommendation: "Inspect and preserve its source; doctor will not delete it automatically" });
    }
  }
  return diagnostics;
}
...
export async function diagnosePipelines(repository: RepositoryDescriptor): Promise<SessionDiagnostic[]> {
  ...
  return pipelines.flatMap((pipeline) => {
    ...
    return [{
      ...
      recommendation: expiredRuns.length ? "Run canopy_pipeline reconcile as the current coordinator; worker source will be preserved" : pipeline.status === "stale_candidate" ? "Inspect integration drift and create or run a new pipeline deliberately" : "Use canopy_pipeline status for source-bound slice and batch receipts",
    }];
  });
}
