...
export function isLikelyCanopyIntent(prompt: string): boolean {
...
export async function inspectCoordinator(repository: RepositoryDescriptor, piSessionId: string): Promise<CoordinatorSnapshot> {
  ...
  if (!runtime) {
    return { initialized: false, route: "initialize", reason: "Canopy runtime is not initialized for this trusted project.", baselineReady: false, activeWriter: false, activePipeline: false, workerRecommendation: unavailableRecommendation("Initialize runtime before planning or implementation.") };
  }
