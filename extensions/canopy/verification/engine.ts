...
export async function verifySession(input: VerificationInput): Promise<VerificationReceipt> {
  ...
  if (project && project.kind !== "swift-package") {
    ...
    if (deployment < 26) throw new SafetyKernelError(`Canopy requires ${project.platform === "macos" ? "macOS" : "iOS"} deployment target 26 or newer, found ${project.deploymentTarget ?? "unknown"}`);
  }
  ...
  try {
    ...
    const artifactRoot = join(input.repository.primaryRoot, ".canopy", "artifacts", "verification");
    ...
    const resources = join(input.repository.primaryRoot, ".canopy", "resources", input.session.id);
