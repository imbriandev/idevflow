...
import type { ApplePlatform, CanopyConfig } from "../config/config.ts";
...
function summaryPath(repository: RepositoryDescriptor): string { return join(repository.primaryRoot, ".canopy", "receipts", "platform-matrix", "latest.json"); }
...
export async function verifyPlatformMatrix(input: {
  ...
  await mkdir(join(input.repository.primaryRoot, ".canopy", "receipts", "platform-matrix"), { recursive: true, mode: 0o700 });
