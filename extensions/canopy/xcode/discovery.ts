...
const IGNORED_DIRECTORIES = new Set([".git", ".canopy", "DerivedData", ".build", "Pods", "Carthage", "node_modules"]);
...
function selectScheme(schemes: readonly string[], configured?: string): string {
  ...
  throw new SafetyKernelError(`Scheme selection is ambiguous: ${schemes.join(", ") || "none"}. Set xcode.scheme in .canopy/config.json`);
}
