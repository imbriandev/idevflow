...
export async function discoverToolchain(cwd: string, probe: CommandProbe = systemProbe): Promise<ToolchainDescriptor> {
  ...
  if (xcodeVersion < 26) throw new SafetyKernelError(`Canopy requires Xcode 26 or newer, found ${descriptor.xcode}`);
  if (swiftVersion < 6.2) throw new SafetyKernelError(`Canopy requires Swift 6.2 or newer, found ${descriptor.swift}`);
