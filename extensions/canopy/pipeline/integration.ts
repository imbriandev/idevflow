...
export async function integratePipelineBatch(
  ...
  return withFileLock(lock, async () => {
    ...
    const branch = `canopy/epoch-${pipelineId}-${suffix}`;
    const root = join(dirname(repository.primaryRoot), `${basename(repository.primaryRoot)}.canopy-epochs`, `${pipelineId}-${suffix}`);
    ...
    try {
      if (primaryBranch !== config.integrationBranch) {
        integrationRoot = join(dirname(repository.primaryRoot), `${basename(repository.primaryRoot)}.canopy-integration`);
