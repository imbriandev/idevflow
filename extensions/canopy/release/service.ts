...
function candidatePath(repository: RepositoryDescriptor): string { return join(repository.primaryRoot, ".canopy", "release", "candidate.json"); }
function approvalPath(repository: RepositoryDescriptor): string { return join(repository.primaryRoot, ".canopy", "approvals", "promotion.json"); }
async function ensureReleaseDirectories(repository: RepositoryDescriptor): Promise<void> {
  await Promise.all([
    mkdir(join(repository.primaryRoot, ".canopy", "release"), { recursive: true, mode: 0o700 }),
    mkdir(join(repository.primaryRoot, ".canopy", "approvals"), { recursive: true, mode: 0o700 }),
  ]);
}
...
async function transition(repository: RepositoryDescriptor, to: "candidate_verified" | "ready_for_ship_approval" | "promoted" | "testflight_handoff" | "stale_candidate", reason: string, actor: string): Promise<void> {
  ...
  if (!state) throw new SafetyKernelError("Canopy runtime is not initialized");
...
async function createCandidateLocked(
  ...
  try { reviewReceipt = JSON.parse(await readFile(join(repository.primaryRoot, ".canopy", "receipts", "stages", `review-${commit}.json`), "utf8")) as typeof reviewReceipt; }
...
export async function createMacDistributionHandoff(
  ...
  try { review = JSON.parse(await readFile(join(repository.primaryRoot, ".canopy", "receipts", "stages", `review-${commit}.json`), "utf8")) as typeof review; } catch (error) { throw new SafetyKernelError("macOS handoff requires the exact review receipt", { cause: error }); }
  ...
  const handoffPath = join(repository.primaryRoot, ".canopy", "release", `macos-handoff-${handoff.id}.json`);
  await ensureReleaseDirectories(repository);
...
export async function createCandidate(
  ...
  const lock = join(repository.primaryRoot, ".canopy", "state", "locks", "integration.lock");
...
async function consumeApproval(repository: RepositoryDescriptor, candidate: ReleaseCandidate, token: string): Promise<ApprovalRecord> {
  ...
  const lock = join(repository.primaryRoot, ".canopy", "state", "locks", "approval.lock");
...
async function promoteCandidateLocked(repository: RepositoryDescriptor, token: string): Promise<ReleaseCandidate> {
  ...
  const config: CanopyConfig = await loadConfig(repository.primaryRoot);
  if (await integrationHead(repository, config) !== candidate.commit) {
  ...
  if (branch !== config.baseBranch) {
    promotionRoot = join(dirname(repository.primaryRoot), `${basename(repository.primaryRoot)}.canopy-promotion`);
...
export async function createTestFlightHandoff(repository: RepositoryDescriptor, acknowledgedBy: string): Promise<{ candidate: ReleaseCandidate; handoffPath: string }> {
  ...
  const existingHandoffPath = join(repository.primaryRoot, ".canopy", "release", `handoff-${candidate.id}.json`);
