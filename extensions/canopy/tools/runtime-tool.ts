...
export function registerRuntimeTool(pi: ExtensionAPI): void {
  pi.registerTool({
    name: "canopy_runtime",
    label: "Canopy Runtime",
    description: "Inspect or initialize the deterministic Canopy project runtime. Initialization creates local .canopy state but does not modify product source.",
    promptSnippet: "Inspect or initialize deterministic Canopy project state",
    promptGuidelines: [
      "Use canopy_runtime status before claiming that Canopy project state or a lifecycle gate exists.",
      "Use canopy_runtime initialize only in a trusted Git project and before future write-capable kernel operations.",
    ],
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (params.action !== "status" && !ctx.isProjectTrusted()) {
        throw new Error(`Refusing to ${params.action} Canopy state in an untrusted project`);
      }
      if (params.action === "migrate") {
        if (!ctx.hasUI) throw new Error("Config migration fails closed without interactive approval");
        const approved = await ctx.ui.confirm("Migrate Canopy config?", "A backup will be written before applying the versioned migration.");
        if (!approved) return { content: [{ type: "text", text: "Canopy config migration cancelled." }], details: { migrated: false } };
      }
      ...
      const text = state
        ? `Canopy runtime: revision ${state.revision}, lifecycle ${state.lifecycle}, repository ${state.repositoryId}; baseline ${baseline.ready ? "ready" : "blocked"}.`
        : "Canopy runtime is not initialized for this repository.";
      return {
    ...
    renderCall(args, theme) {
      return new Text(
        `${theme.fg("toolTitle", theme.bold("canopy_runtime "))}${theme.fg("accent", args.action)}`,
        0,
        0,
      );
    },
