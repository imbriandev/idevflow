...
export function registerPreflightTool(pi: ExtensionAPI, readState: () => SessionState): void {
  pi.registerTool({
    name: "canopy_preflight",
    label: "Canopy Preflight",
    description: "Validate a Canopy stage and, for writes, create or reuse an isolated writer worktree with exclusive path claims.",
    promptSnippet: "Authorize Canopy writes and allocate a claimed worktree",
    promptGuidelines: [
      "Call canopy_preflight with write=true before edit or write in an active Canopy stage.",
      "After write preflight, use the returned worktree; built-in edit/write paths are redirected and checked by the Canopy gate.",
    ],
    ...
    async execute(_id, params, _signal, _update, ctx) {
      ...
      const workerPacketPath = process.env.CANOPY_WORKER_PACKET;
      if (workerPacketPath) {
        const discovered = await discoverRepository(ctx.cwd);
        const packet = await readWorkerPacket(await assertPacketPath(workerPacketPath, discovered.primaryRoot), process.env.CANOPY_WORKER_PACKET_DIGEST);
