...
export function registerPipelineTool(pi: ExtensionAPI, extensionPath: string): void {
  pi.registerTool({
    name: "canopy_pipeline",
    label: "Canopy Pipeline",
    ...
    promptGuidelines: [
      "Use canopy_pipeline create only for the exact approved plan, then run dependency-ready workers under bounded concurrency.",
      "Use canopy_pipeline reconcile after reload or interruption; never infer worker completion from prose or process exit alone.",
    ...
    async execute(_id, params, signal, onUpdate, ctx) {
      if (process.env.CANOPY_WORKER_PACKET) throw new SafetyKernelError("Pipeline workers cannot invoke coordinator operations");
