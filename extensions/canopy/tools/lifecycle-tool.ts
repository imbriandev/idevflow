...
export function registerLifecycleTool(pi: ExtensionAPI): void {
  pi.registerTool({
    name: "canopy_lifecycle",
    label: "Canopy Lifecycle",
    ...
    async execute(_id, params, _signal, _update, ctx) {
      ...
      if (params.action === "status") {
        ...
        return { content: [{ type: "text", text: state ? `Lifecycle ${state.lifecycle} at revision ${state.revision}${product ? `; product ${product.fingerprint}` : ""}${graph ? `; graph ${graph.fingerprint}` : ""}.` : "Canopy runtime is not initialized." }], details: { state, product, graph } };
      }
