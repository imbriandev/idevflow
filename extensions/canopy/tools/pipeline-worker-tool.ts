...
export function registerPipelineWorkerTool(pi: ExtensionAPI): void {
  pi.registerTool({
    name: "canopy_pipeline_worker",
    label: "Canopy Pipeline Worker",
    description: "Report heartbeat, bounded repair, block evidence, or exact completion from an isolated Canopy worker process.",
    ...
    async execute(_id, params, _signal, _update, ctx) {
      if (!process.env.CANOPY_WORKER_PACKET) throw new SafetyKernelError("canopy_pipeline_worker is available only inside an authorized worker process");
