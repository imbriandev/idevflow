  ...
  pi.registerTool({
    ...
    async execute(_id, params, _signal, _update, ctx) {
      ...
      if (params.action === "report") {
        ...
        return { content: [{ type: "text", text: `Canopy diagnostic report: ${report.health}; ${report.sessions.total} writer session(s), ${report.pipelines.total} pipeline(s), ${report.diagnostics.length} diagnostic(s).` }], details: { report } };
      }
      if (params.action === "repair") {
        ...
        const approved = await ctx.ui.confirm("Repair Canopy registry?", "Expired sessions will be marked stale. No branch or worktree will be deleted.");
