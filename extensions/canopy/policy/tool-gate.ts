...
async function packageReferencePath(input: string): Promise<string | undefined> {
  const extension = process.env.CANOPY_WORKER_EXTENSION;
...
export function registerToolGate(pi: ExtensionAPI, readState: () => SessionState): void {
  pi.on("tool_call", async (event, ctx) => {
    const workerMode = Boolean(process.env.CANOPY_WORKER_PACKET);
    const stage = readState().stage ?? (workerMode ? "build" : undefined);
    if (!stage) return;
    const contract = STAGE_CONTRACTS[stage];

    if (workerMode && ["canopy_runtime", "canopy_lifecycle", "canopy_release", "canopy_pipeline", "canopy_doctor", "canopy_simulator", "canopy_proof"].includes(event.toolName)) {
    ...
    if (event.toolName === "edit" || event.toolName === "write") {
      ...
      if (!session || session.status !== "active") {
        return { block: true, reason: "Canopy write blocked: run canopy_preflight with write=true first." };
      }
      if (!leaseIsValid(session)) {
        return { block: true, reason: `Canopy write blocked: session ${session.id} lease expired; run doctor repair and resume explicitly.` };
      }
      const input = event.input as { path?: string };
      if (typeof input.path !== "string") return { block: true, reason: "Canopy write blocked: missing path." };
      let resolved: { absolute: string; projectPath: string };
      try {
        resolved = await resolveSafeWritePath(input.path, session.worktreePath);
      } catch (error) {
        return { block: true, reason: `Canopy write blocked: ${(error as Error).message}` };
      }
      if (!pathIsClaimed(resolved.projectPath, session.claims)) {
        return { block: true, reason: `Canopy write blocked: ${resolved.projectPath} is outside claims ${session.claims.join(", ")}.` };
      }
    ...
    if (event.toolName === "bash") {
      ...
      if (!decision.allowed) {
        return { block: true, reason: `Canopy shell gate: ${decision.reason}. Use canopy_exec for allowlisted build/test operations.` };
      }
