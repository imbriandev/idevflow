...
export async function createWriterWorktree(input: {
  ...
  const branch = `canopy/${slugify(input.task)}-${shortId}`;
  const defaultParent = join(dirname(input.repository.primaryRoot), `${basename(input.repository.primaryRoot)}.canopy-worktrees`);
