...
export async function integratePipelineBatch(
  ...
  return withFileLock(lock, async () => {
    ...
    const branch = `pi-ios/epoch-${pipelineId}-${suffix}`;
    const root = join(dirname(repository.primaryRoot), `${basename(repository.primaryRoot)}.pi-ios-epochs`, `${pipelineId}-${suffix}`);
    ...
    try {
      if (primaryBranch !== config.integrationBranch) {
        integrationRoot = join(dirname(repository.primaryRoot), `${basename(repository.primaryRoot)}.pi-ios-integration`);
