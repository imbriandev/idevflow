...
async function writeStageReceipt(repository: RepositoryDescriptor, receipt: StageReceipt): Promise<void> {
  await mkdir(join(repository.primaryRoot, ".appforge", "receipts", "stages"), { recursive: true, mode: 0o700 });
  await writeFileAtomically(join(repository.primaryRoot, ".appforge", "receipts", "stages", `${receipt.stage}-${receipt.sourceCommit}.json`), `${JSON.stringify(receipt, null, 2)}\n`);
}
...
async function completedSlices(repository: RepositoryDescriptor, graphFingerprint: string): Promise<Set<string>> {
  const directory = join(repository.primaryRoot, ".appforge", "receipts", "stages");
  let names: string[];
...
async function readApproval(repository: RepositoryDescriptor): Promise<PlanApproval> {
  try { return JSON.parse(await readFile(join(repository.primaryRoot, ".appforge", "approvals", "plan.json"), "utf8")) as PlanApproval; }
...
export async function approvePlan(repository: RepositoryDescriptor, approvedBy: string): Promise<PlanApproval> {
  ...
  await mkdir(join(repository.primaryRoot, ".appforge", "approvals"), { recursive: true, mode: 0o700 });
  await writeFileAtomically(join(repository.primaryRoot, ".appforge", "approvals", "plan.json"), `${JSON.stringify(approval, null, 2)}\n`);
  await transition(repository, "plan_approved", `approved work graph ${graph.fingerprint}`, `approval:${approvedBy}`);
  return approval;
}

export async function integrateCurrentStage(repository: RepositoryDescriptor, session: WriterSession, evidence: string, requestedSliceId?: string): Promise<StageReceipt> {
  ...
  const receipt: StageReceipt = {
    ...
    verificationFingerprint, evidence: evidence.trim(), ...(graphFingerprint ? { graphFingerprint } : {}), ...(sliceId ? { sliceId } : {}), sessionId: session.id, integration, recordedAt: new Date().toISOString(),
  };
