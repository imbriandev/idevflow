...
function candidatePath(repository: RepositoryDescriptor): string { return join(repository.primaryRoot, ".appforge", "release", "candidate.json"); }
function approvalPath(repository: RepositoryDescriptor): string { return join(repository.primaryRoot, ".appforge", "approvals", "promotion.json"); }
async function ensureReleaseDirectories(repository: RepositoryDescriptor): Promise<void> {
  await Promise.all([
    mkdir(join(repository.primaryRoot, ".appforge", "release"), { recursive: true, mode: 0o700 }),
    mkdir(join(repository.primaryRoot, ".appforge", "approvals"), { recursive: true, mode: 0o700 }),
  ]);
}
...
async function transition(repository: RepositoryDescriptor, to: "candidate_verified" | "ready_for_ship_approval" | "promoted" | "testflight_handoff" | "stale_candidate", reason: string, actor: string): Promise<void> {
  ...
  if (!state) throw new SafetyKernelError("Pi iOS runtime is not initialized");
...
async function createCandidateLocked(
  ...
  try { reviewReceipt = JSON.parse(await readFile(join(repository.primaryRoot, ".appforge", "receipts", "stages", `review-${commit}.json`), "utf8")) as typeof reviewReceipt; }
...
export async function createCandidate(
  ...
  const lock = join(repository.primaryRoot, ".appforge", "state", "locks", "integration.lock");
...
async function consumeApproval(repository: RepositoryDescriptor, candidate: ReleaseCandidate, token: string): Promise<ApprovalRecord> {
  ...
  const lock = join(repository.primaryRoot, ".appforge", "state", "locks", "approval.lock");
...
async function promoteCandidateLocked(repository: RepositoryDescriptor, token: string): Promise<ReleaseCandidate> {
  ...
  const config: PiIosConfig = await loadConfig(repository.primaryRoot);
  if (await integrationHead(repository, config) !== candidate.commit) {
  ...
  if (branch !== config.baseBranch) {
    promotionRoot = join(dirname(repository.primaryRoot), `${basename(repository.primaryRoot)}.pi-ios-promotion`);
...
export async function createTestFlightHandoff(repository: RepositoryDescriptor, acknowledgedBy: string): Promise<{ candidate: ReleaseCandidate; handoffPath: string }> {
  ...
  const existingHandoffPath = join(repository.primaryRoot, ".appforge", "release", `handoff-${candidate.id}.json`);
