...
export function registerPreflightTool(pi: ExtensionAPI, readState: () => SessionState): void {
  pi.registerTool({
    name: "pi_ios_preflight",
    label: "Pi iOS Preflight",
    description: "Validate a Pi iOS stage and, for writes, create or reuse an isolated writer worktree with exclusive path claims.",
    promptSnippet: "Authorize Pi iOS writes and allocate a claimed worktree",
    promptGuidelines: [
      "Call pi_ios_preflight with write=true before edit or write in an active Pi iOS stage.",
      "After write preflight, use the returned worktree; built-in edit/write paths are redirected and checked by the Pi iOS gate.",
    ],
