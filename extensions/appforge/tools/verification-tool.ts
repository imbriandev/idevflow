  ...
  pi.registerTool({
    ...
    async execute(_id, params, signal, onUpdate, ctx) {
      ...
      for (const proof of params.proofs ?? []) {
        ...
        const runtimeRoot = await realpath(resolve(repository.primaryRoot, ".appforge"));
        ...
        if ((!isWithin(worktreeRoot, canonicalPath) && !isWithin(runtimeRoot, canonicalPath)) || (!isWithin(worktreeRoot, canonicalMetadataPath) && !isWithin(runtimeRoot, canonicalMetadataPath))) {
          throw new SafetyKernelError("Proof and metadata paths must stay inside the writer worktree or local .appforge runtime");
        }
