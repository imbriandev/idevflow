...
export async function integrationHead(repository: RepositoryDescriptor, config: PiIosConfig): Promise<string> {
...
export async function integrateSession(repository: RepositoryDescriptor, config: PiIosConfig, session: WriterSession): Promise<IntegrationReceipt> {
  ...
  const lockPath = join(repository.primaryRoot, ".appforge", "state", "locks", "integration.lock");
  return withFileLock(lockPath, async () => {
    ...
    if (currentBranch !== config.integrationBranch) {
      const parent = join(dirname(repository.primaryRoot), `${basename(repository.primaryRoot)}.pi-ios-integration`);
    ...
    try {
      ...
      await mkdir(join(repository.primaryRoot, ".appforge", "receipts", "integration"), { recursive: true, mode: 0o700 });
      await writeFileAtomically(join(repository.primaryRoot, ".appforge", "receipts", "integration", `${session.id}.json`), `${JSON.stringify(receipt, null, 2)}\n`);
