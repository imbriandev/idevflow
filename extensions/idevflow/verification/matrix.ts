...
function summaryPath(repository: RepositoryDescriptor): string { return join(repository.primaryRoot, ".idevflow", "receipts", "platform-matrix", "latest.json"); }
...
export async function verifyPlatformMatrix(input: {
  ...
  await mkdir(join(input.repository.primaryRoot, ".idevflow", "receipts", "platform-matrix"), { recursive: true, mode: 0o700 });
