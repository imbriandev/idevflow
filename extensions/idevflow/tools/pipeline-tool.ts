...
export function registerPipelineTool(pi: ExtensionAPI, extensionPath: string): void {
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use idev_pipeline create only for the exact approved plan, then run dependency-ready workers under bounded concurrency.",
      "Use idev_pipeline reconcile after reload or interruption; never infer worker completion from prose or process exit alone.",
    ...
    async execute(_id, params, signal, onUpdate, ctx) {
      if (process.env.IDEVFLOW_WORKER_PACKET) throw new SafetyKernelError("Pipeline workers cannot invoke coordinator operations");
