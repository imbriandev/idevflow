...
export function registerSimulatorTool(pi: ExtensionAPI): void {
  pi.registerTool({
    name: "idev_simulator",
    label: "iDevFlow Simulator",
    description: "Inspect, acquire, boot, or release the exclusive simulator lease for the current iDevFlow writer session.",
    ...
    async execute(_id, params, _signal, _update, ctx) {
      ...
      if (!session) throw new SafetyKernelError("Simulator operations require a iDevFlow writer session");
