...
export function registerVerificationTool(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use idev_verify before idev_session postflight for every write-capable iDevFlow session.",
    ...
    async execute(_id, params, signal, onUpdate, ctx) {
      ...
      for (const proof of params.proofs ?? []) {
        ...
        const runtimeRoot = await realpath(resolve(repository.primaryRoot, ".idevflow"));
        ...
        if ((!isWithin(worktreeRoot, canonicalPath) && !isWithin(runtimeRoot, canonicalPath)) || (!isWithin(worktreeRoot, canonicalMetadataPath) && !isWithin(runtimeRoot, canonicalMetadataPath))) {
          throw new SafetyKernelError("Proof and metadata paths must stay inside the writer worktree or local .idevflow runtime");
        }
