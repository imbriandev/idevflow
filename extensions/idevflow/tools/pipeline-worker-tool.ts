...
export function registerPipelineWorkerTool(pi: ExtensionAPI): void {
  pi.registerTool({
    name: "idev_pipeline_worker",
    label: "iDevFlow Pipeline Worker",
    description: "Report heartbeat, bounded repair, block evidence, or exact completion from an isolated iDevFlow worker process.",
    ...
    async execute(_id, params, _signal, _update, ctx) {
      if (!process.env.IDEVFLOW_WORKER_PACKET) throw new SafetyKernelError("idev_pipeline_worker is available only inside an authorized worker process");
