...
export function registerContextTool(pi: ExtensionAPI): void {
  pi.registerTool({
    name: "idev_context",
    label: "iDevFlow Specialist Context",
    description: "Select a bounded, stage- and surface-specific set of iDevFlow specialist references without loading unrelated context.",
    promptSnippet: "Select bounded specialist iOS references before complex implementation or review",
    promptGuidelines: ["Use idev_context before loading specialist references for SwiftUI, SwiftData, concurrency, testing, accessibility, privacy, monetization, release, widgets, or App Intents work; read only the returned paths that apply."],
    ...
    async execute(_id, parameters, _signal, _update, ctx) {
      ...
      if (ctx.isProjectTrusted()) {
        ...
        if (session && ["active", "ready_for_integration", "integrated"].includes(session.status)) {
          const eligible = parameters.stage === session.stage || (parameters.stage === "review" && session.status !== "active") || (parameters.stage === "ship" && session.status !== "active");
          if (eligible) {
