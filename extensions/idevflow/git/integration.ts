...
export async function integrationHead(repository: RepositoryDescriptor, config: iDevFlowConfig): Promise<string> {
...
export async function integrateSession(repository: RepositoryDescriptor, config: iDevFlowConfig, session: WriterSession): Promise<IntegrationReceipt> {
  ...
  const lockPath = join(repository.primaryRoot, ".idevflow", "state", "locks", "integration.lock");
  return withFileLock(lockPath, async () => {
    ...
    if (currentBranch !== config.integrationBranch) {
      const parent = join(dirname(repository.primaryRoot), `${basename(repository.primaryRoot)}.idev-integration`);
    ...
    try {
      ...
      await mkdir(join(repository.primaryRoot, ".idevflow", "receipts", "integration"), { recursive: true, mode: 0o700 });
      await writeFileAtomically(join(repository.primaryRoot, ".idevflow", "receipts", "integration", `${session.id}.json`), `${JSON.stringify(receipt, null, 2)}\n`);
