...
async function context(repository: RepositoryDescriptor): Promise<{ packet: WorkerTaskPacket; capability: string; config: iDevFlowConfig }> {
  const path = process.env.IDEVFLOW_WORKER_PACKET;
  const capability = process.env.IDEVFLOW_WORKER_CAPABILITY;
  if (!path || !capability) throw new SafetyKernelError("Pipeline worker capability context is unavailable");
  const packet = await readWorkerPacket(await assertPacketPath(path, repository.primaryRoot), process.env.IDEVFLOW_WORKER_PACKET_DIGEST);
...
export async function heartbeatPipelineWorker(repository: RepositoryDescriptor, piSessionId: string): Promise<void> {
  if (!process.env.IDEVFLOW_WORKER_PACKET) return;
