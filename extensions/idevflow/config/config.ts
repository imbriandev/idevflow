...
export const DEFAULT_CONFIG: iDevFlowConfig = {
  ...
  documents: {
    ...
    workGraph: "docs/idevflow/work-graph.json",
    privacyReview: "docs/idevflow/privacy-review.json",
...
export async function migrateLegacyRuntimeDirectory(primaryRoot: string): Promise<void> {
  const current = join(primaryRoot, ".idevflow");
  const legacyPaths = [".canopy", ".pi-ios"].map((name) => join(primaryRoot, name));
  const existingLegacy: string[] = [];
  for (const path of legacyPaths) {
    try {
      const info = await lstat(path);
      if (info.isSymbolicLink()) throw new SafetyKernelError("Refusing to migrate a symbolic-link legacy runtime directory");
      existingLegacy.push(path);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
  }
  if (!existingLegacy.length) return;
  try { await lstat(current); throw new SafetyKernelError("A legacy iDevFlow runtime and .idevflow both exist; remove neither and reconcile them manually"); }
  catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; }
  if (existingLegacy.length > 1) throw new SafetyKernelError("Multiple legacy iDevFlow runtime directories exist; remove neither and reconcile them manually");
  await rename(existingLegacy[0]!, current);
}

export function configPath(primaryRoot: string): string {
  return join(primaryRoot, ".idevflow", "config.json");
}

function optionalString(value: unknown, name: string): string | undefined {
  if (value === undefined) return undefined;
  if (typeof value !== "string" || !value.trim()) throw new SafetyKernelError(`iDevFlow config ${name} must be a non-empty string when set`);
  return value;
}

function positiveInteger(value: unknown, name: string, minimum = 1): number {
  if (!Number.isInteger(value) || (value as number) < minimum) {
    throw new SafetyKernelError(`iDevFlow config ${name} must be an integer of at least ${minimum}`);
  }
  return value as number;
}

export function validateConfig(value: unknown): iDevFlowConfig {
  if (!value || typeof value !== "object") throw new SafetyKernelError("iDevFlow config must be a JSON object");
  const config = value as Partial<iDevFlowConfig>;
  if (config.schemaVersion !== CONFIG_SCHEMA_VERSION) {
    throw new SafetyKernelError(`Unsupported iDevFlow config schema: ${String(config.schemaVersion)}`);
  }
  ...
  if (!config.xcode || typeof config.xcode !== "object") throw new SafetyKernelError("iDevFlow config xcode must be an object");
  if (config.xcode.platform !== "ios" && config.xcode.platform !== "macos") throw new SafetyKernelError("iDevFlow config xcode.platform must be ios or macos");
  if (!Array.isArray(config.xcode.requiredPlatforms) || config.xcode.requiredPlatforms.length === 0 || config.xcode.requiredPlatforms.some((platform) => platform !== "ios" && platform !== "macos") || new Set(config.xcode.requiredPlatforms).size !== config.xcode.requiredPlatforms.length || !config.xcode.requiredPlatforms.includes(config.xcode.platform)) throw new SafetyKernelError("iDevFlow config xcode.requiredPlatforms must contain unique ios/macos values including xcode.platform");
  ...
  if (!config.simulator || typeof config.simulator !== "object") throw new SafetyKernelError("iDevFlow config simulator must be an object");
  ...
  if (typeof config.simulator.keepBooted !== "boolean") throw new SafetyKernelError("iDevFlow config simulator.keepBooted must be boolean");
  if (simulatorLeaseSeconds < verificationTimeoutSeconds * 2 + 300) {
    throw new SafetyKernelError("iDevFlow simulator lease must cover two verification actions plus a 300-second safety margin");
  }

  if (!config.verification || typeof config.verification !== "object") throw new SafetyKernelError("iDevFlow config verification must be an object");
  ...
  if (typeof config.quality.requireXCTestEvidence !== "boolean") throw new SafetyKernelError("iDevFlow config quality.requireXCTestEvidence must be boolean");
  if (!config.quality.performanceBudgets || typeof config.quality.performanceBudgets !== "object" || Array.isArray(config.quality.performanceBudgets)) throw new SafetyKernelError("iDevFlow config quality.performanceBudgets must be an object");
  for (const [metric, budget] of Object.entries(config.quality.performanceBudgets)) {
    if (!metric.trim() || !Number.isFinite(budget) || budget <= 0) throw new SafetyKernelError("iDevFlow config quality.performanceBudgets requires non-empty metric names and positive finite budgets");
  }

  if (!config.documents || typeof config.documents !== "object") throw new SafetyKernelError("iDevFlow config documents must be an object");
  for (const key of ["productMemory", "slcSpec", "workGraph", "privacyReview", "monetization", "releaseManifest"] as const) {
    ...
    if (path.startsWith("/") || path.split(/[\\/]/).includes("..")) throw new SafetyKernelError(`iDevFlow config documents.${key} must stay project-relative`);
  }
  if (!config.release || typeof config.release !== "object") throw new SafetyKernelError("iDevFlow config release must be an object");
...
export async function loadConfig(primaryRoot: string): Promise<iDevFlowConfig> {
...
export async function applyConfigMigration(primaryRoot: string): Promise<iDevFlowConfig> {
...
export async function initializeConfig(primaryRoot: string): Promise<iDevFlowConfig> {
  await migrateLegacyRuntimeDirectory(primaryRoot);
  const path = configPath(primaryRoot);
  await mkdir(join(primaryRoot, ".idevflow"), { recursive: true, mode: 0o700 });
  try {
    await access(path);
