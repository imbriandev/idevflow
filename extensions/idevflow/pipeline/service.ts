...
export class PipelineService {
  ...
  private async reserveWorker(pipeline: PipelineState, sliceId: string, piSessionId: string, config: iDevFlowConfig, options: { readonly model?: string; readonly thinkingLevel?: string; readonly signal?: AbortSignal }): Promise<Dispatch> {
    const slice = pipeline.slices[sliceId]!;
    ...
    const packetPath = join(this.repository.primaryRoot, ".idevflow", "pipeline", "packets", `${packetId}.json`);
    await mkdir(dirname(packetPath), { recursive: true, mode: 0o700 });
    const packetHash = await writeWorkerPacket(packetPath, packet);
    const logRoot = join(this.repository.primaryRoot, ".idevflow", "logs", "workers", runId);
    const now = Date.now();
  ...
  private async integrateWithSplitting(id: string, sliceIds: readonly string[], sourceEpoch: string, piSessionId: string, config: iDevFlowConfig): Promise<PipelineState> {
    let pipeline = await this.require(id);
  ...
  private async finalizeCandidate(pipeline: PipelineState, piSessionId: string, config: iDevFlowConfig, signal?: AbortSignal, onProgress?: (message: string) => void): Promise<PipelineState> {
    ...
    if (existing && await git(existing.worktreePath, ["rev-parse", "HEAD"]).catch(() => "") === commit && !(await git(existing.worktreePath, ["status", "--porcelain=v1"]).catch(() => "missing"))) {
      ...
      const parent = config.pipeline.candidateWorktreeDirectory ? (isAbsolute(config.pipeline.candidateWorktreeDirectory) ? config.pipeline.candidateWorktreeDirectory : resolve(this.repository.primaryRoot, config.pipeline.candidateWorktreeDirectory)) : join(dirname(this.repository.primaryRoot), `${basename(this.repository.primaryRoot)}.idev-candidates`);
      const candidateId = randomUUID();
      const worktree = join(parent, `${pipeline.id}-${candidateId.slice(0, 8)}`);
      const branch = `idev/candidate-${pipeline.id}-${candidateId.slice(0, 8)}`;
  ...
  private async writeCombinedLifecycleReceipts(pipeline: PipelineState, candidate: PipelineCandidateSnapshot, verificationFingerprint: string): Promise<void> {
    const directory = join(this.repository.primaryRoot, ".idevflow", "receipts", "stages");
  ...
  private async currentGraph(config: iDevFlowConfig): Promise<ValidatedWorkGraph> {
  ...
  private async readPlanApproval(): Promise<PlanApproval> {
    try { return JSON.parse(await readFile(join(this.repository.primaryRoot, ".idevflow", "approvals", "plan.json"), "utf8")) as PlanApproval; }
