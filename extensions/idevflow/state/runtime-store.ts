...
import { migrateLegacyRuntimeDirectory } from "../config/config.ts";
...
export class RuntimeStore {
  ...
  constructor(
    ...
    this.runtimeRoot = join(repository.primaryRoot, ".idevflow");
  ...
  async status(): Promise<RuntimeState | null> {
    await migrateLegacyRuntimeDirectory(this.repository.primaryRoot);
    try {
