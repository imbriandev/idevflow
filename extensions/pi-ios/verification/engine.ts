...
export async function verifySession(input: VerificationInput): Promise<VerificationReceipt> {
  ...
  try {
    ...
    const artifactRoot = join(input.repository.primaryRoot, ".pi-ios", "artifacts", "verification");
    ...
    const resources = join(input.repository.primaryRoot, ".pi-ios", "resources", input.session.id);
