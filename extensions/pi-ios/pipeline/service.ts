...
export class PipelineService {
  ...
  private async reserveWorker(pipeline: PipelineState, sliceId: string, piSessionId: string, config: PiIosConfig, options: { readonly model?: string; readonly thinkingLevel?: string; readonly signal?: AbortSignal }): Promise<Dispatch> {
    ...
    const packetPath = join(this.repository.primaryRoot, ".pi-ios", "pipeline", "packets", `${packetId}.json`);
    await mkdir(dirname(packetPath), { recursive: true, mode: 0o700 });
    const packetHash = await writeWorkerPacket(packetPath, packet);
    const logRoot = join(this.repository.primaryRoot, ".pi-ios", "logs", "workers", runId);
    const now = Date.now();
  ...
  private async writeCombinedLifecycleReceipts(pipeline: PipelineState, candidate: PipelineCandidateSnapshot, verificationFingerprint: string): Promise<void> {
    const directory = join(this.repository.primaryRoot, ".pi-ios", "receipts", "stages");
  ...
  private async readPlanApproval(): Promise<PlanApproval> {
    try { return JSON.parse(await readFile(join(this.repository.primaryRoot, ".pi-ios", "approvals", "plan.json"), "utf8")) as PlanApproval; }
