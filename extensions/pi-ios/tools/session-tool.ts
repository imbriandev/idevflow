...
export function registerSessionTool(pi: ExtensionAPI): void {
  pi.registerTool({
    name: "pi_ios_session",
    label: "Pi iOS Session",
    description: "Inspect, heartbeat, park, postflight, or finish the current isolated Pi iOS writer session.",
    promptSnippet: "Manage Pi iOS writer heartbeat, postflight, and finish",
    promptGuidelines: [
      "Call pi_ios_session postflight with concise evidence after all source changes and checks are complete.",
      "Call pi_ios_session finish only after postflight and without modifying files afterward.",
    ],
    ...
    async execute(_id, params, _signal, _update, ctx) {
      ...
      if (!session) throw new SafetyKernelError("No current Pi iOS writer session");
