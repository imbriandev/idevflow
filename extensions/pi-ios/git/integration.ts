...
export async function integrateSession(repository: RepositoryDescriptor, config: PiIosConfig, session: WriterSession): Promise<IntegrationReceipt> {
  ...
  const lockPath = join(repository.primaryRoot, ".pi-ios", "state", "locks", "integration.lock");
  return withFileLock(lockPath, async () => {
    ...
    try {
      ...
      await mkdir(join(repository.primaryRoot, ".pi-ios", "receipts", "integration"), { recursive: true, mode: 0o700 });
      await writeFileAtomically(join(repository.primaryRoot, ".pi-ios", "receipts", "integration", `${session.id}.json`), `${JSON.stringify(receipt, null, 2)}\n`);
