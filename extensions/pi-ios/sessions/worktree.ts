...
export async function createWriterWorktree(input: {
  ...
  const branch = `pi-ios/${slugify(input.task)}-${shortId}`;
  const defaultParent = join(dirname(input.repository.primaryRoot), `${basename(input.repository.primaryRoot)}.pi-ios-worktrees`);
