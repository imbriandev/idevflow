...
function candidatePath(repository: RepositoryDescriptor): string { return join(repository.primaryRoot, ".pi-ios", "release", "candidate.json"); }
function approvalPath(repository: RepositoryDescriptor): string { return join(repository.primaryRoot, ".pi-ios", "approvals", "promotion.json"); }
async function ensureReleaseDirectories(repository: RepositoryDescriptor): Promise<void> {
  await Promise.all([
    mkdir(join(repository.primaryRoot, ".pi-ios", "release"), { recursive: true, mode: 0o700 }),
    mkdir(join(repository.primaryRoot, ".pi-ios", "approvals"), { recursive: true, mode: 0o700 }),
  ]);
}
...
async function createCandidateLocked(
  ...
  try { reviewReceipt = JSON.parse(await readFile(join(repository.primaryRoot, ".pi-ios", "receipts", "stages", `review-${commit}.json`), "utf8")) as typeof reviewReceipt; }
...
export async function createCandidate(
  ...
  const lock = join(repository.primaryRoot, ".pi-ios", "state", "locks", "integration.lock");
...
async function consumeApproval(repository: RepositoryDescriptor, candidate: ReleaseCandidate, token: string): Promise<ApprovalRecord> {
  ...
  const lock = join(repository.primaryRoot, ".pi-ios", "state", "locks", "approval.lock");
...
export async function createTestFlightHandoff(repository: RepositoryDescriptor, acknowledgedBy: string): Promise<{ candidate: ReleaseCandidate; handoffPath: string }> {
  ...
  const existingHandoffPath = join(repository.primaryRoot, ".pi-ios", "release", `handoff-${candidate.id}.json`);
