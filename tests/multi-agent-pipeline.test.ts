...
import { integratePipelineBatch } from "../extensions/pi-ios/pipeline/integration.ts";
...
import type { PipelineStageReceipts } from "../extensions/pi-ios/pipeline/types.ts";
import { loadWorkGraph } from "../extensions/pi-ios/planning/work-graph.ts";
import type { SupervisedProcessResult } from "../extensions/pi-ios/process/supervisor.ts";
import { discoverRepository, type RepositoryDescriptor } from "../extensions/pi-ios/repository/discovery.ts";
import { SessionRegistry } from "../extensions/pi-ios/sessions/registry.ts";
import { finishSession, receiptFingerprint, runPostflight, writePreflight } from "../extensions/pi-ios/sessions/service.ts";
...
import type { VerificationInput } from "../extensions/pi-ios/verification/engine.ts";
...
import { readWorkerPacket } from "../extensions/pi-ios/workers/packets.ts";
import type { WorkerLauncher, WorkerLaunchInput } from "../extensions/pi-ios/workers/runner.ts";
...
describe("multi-agent pipeline", () => {
  it("runs independent slices concurrently, integrates one epoch, and verifies a combined candidate", async () => {
    ...
    await mkdir(join(fixture.root, ".pi-ios", "approvals"), { recursive: true }); await writeFile(join(fixture.root, ".pi-ios", "approvals", "plan.json"), JSON.stringify({ graphFingerprint: graph.fingerprint, planCommit: commit }));
