...
import { integratePipelineBatch } from "../extensions/appforge/pipeline/integration.ts";
import { PipelineService } from "../extensions/appforge/pipeline/service.ts";
import { PipelineStore } from "../extensions/appforge/pipeline/store.ts";
import type { PipelineStageReceipts } from "../extensions/appforge/pipeline/types.ts";
import { loadWorkGraph } from "../extensions/appforge/planning/work-graph.ts";
import type { SupervisedProcessResult } from "../extensions/appforge/process/supervisor.ts";
import { discoverRepository, type RepositoryDescriptor } from "../extensions/appforge/repository/discovery.ts";
import { SessionRegistry } from "../extensions/appforge/sessions/registry.ts";
import { finishSession, receiptFingerprint, runPostflight, writePreflight } from "../extensions/appforge/sessions/service.ts";
...
import type { VerificationInput } from "../extensions/appforge/verification/engine.ts";
...
import type { VerificationReceipt } from "../extensions/appforge/verification/types.ts";
import { readWorkerPacket } from "../extensions/appforge/workers/packets.ts";
import type { WorkerLauncher, WorkerLaunchInput } from "../extensions/appforge/workers/runner.ts";
...
describe("multi-agent pipeline", () => {
  it("runs independent slices concurrently, integrates one epoch, and verifies a combined candidate", async () => {
    ...
    for (const suffix of [".pi-ios-worktrees", ".pi-ios-integration", ".pi-ios-epochs", ".pi-ios-candidates"]) cleanups.push(async () => rm(`${fixture.root}${suffix}`, { recursive: true, force: true }));
    ...
    await mkdir(join(fixture.root, "docs/pi-ios"), { recursive: true });
    ...
    await mkdir(join(fixture.root, ".appforge", "approvals"), { recursive: true }); await writeFile(join(fixture.root, ".appforge", "approvals", "plan.json"), JSON.stringify({ graphFingerprint: graph.fingerprint, planCommit: commit }));
