...
import type { VerificationProfile } from "../extensions/idevflow/verification/profiles.ts";
import { VerificationReceiptStore } from "../extensions/idevflow/verification/receipts.ts";
import type { ArtifactRecord, QualityProof, VerificationReceipt } from "../extensions/idevflow/verification/types.ts";
...
async function fakeVerification(repository: Awaited<ReturnType<typeof discoverRepository>>, session: WriterSession, profile: VerificationProfile, project = false): Promise<string> {
  ...
  if (project && profile === "release") {
    const directory = join(repository.primaryRoot, ".idevflow", "artifacts", "fake-release", fingerprint);
...
describe("single-agent full lifecycle", () => {
  it("moves a frozen SLC through approval, receipts, candidate promotion, and manual TestFlight handoff", async () => {
    ...
    cleanups.push(fixture.cleanup, async () => rm(`${fixture.root}.idev-worktrees`, { recursive: true, force: true }), async () => rm(`${fixture.root}.idev-integration`, { recursive: true, force: true }));
    ...
    const plan = await completeWriterStage(repository, {
      piSessionId: "golden", stage: "plan", task: "plan golden app", paths: ["docs/idevflow/work-graph.json"],
      async write(session) {
        await writeFile(join(session.worktreePath, "docs/idevflow/work-graph.json"), JSON.stringify({ schemaVersion: 1, title: "Golden graph", sourceSpecFingerprint: product.fingerprint, architecture: [{ id: "ADR-1", title: "Local", decision: "Keep state local", rationale: "Safety", status: "accepted" }], slices: [{ id: "slice-1", title: "Golden app", goal: "Create release inputs", paths: ["Sources", "docs/idevflow/privacy-review.json", "docs/idevflow/release.json"], risk: "medium", dependsOn: [], acceptance: ["App and release evidence exist"], verificationProfile: "integration" }] }));
      },
    });
    ...
    await completeWriterStage(repository, {
      piSessionId: "golden", stage: "build", task: "build golden slice", paths: ["Sources", "docs/idevflow/privacy-review.json", "docs/idevflow/release.json"],
      async write(session) {
        ...
        await writeFile(join(session.worktreePath, "docs/idevflow/privacy-review.json"), JSON.stringify({ schemaVersion: 1, decision: "go", dataPractices: [], permissions: [], findings: [] }));
        await writeFile(join(session.worktreePath, "docs/idevflow/release.json"), JSON.stringify({ schemaVersion: 1, version: "1.0", build: "1", bundleId: "com.example.golden", target: "testflight-internal", releaseNotes: "Golden beta", knownIssues: [], supportUrl: "https://example.com/support", privacyUrl: "https://example.com/privacy" }));
      },
    });
    ...
    await execFileAsync("git", ["checkout", "idev/integration"], { cwd: fixture.root });
    ...
    await completeWriterStage(repository, {
      piSessionId: "golden", stage: "learn", task: "record beta learning", paths: ["docs/idevflow/product-memory.json"],
      async write(session) {
        ...
        await writeFile(join(session.worktreePath, "docs/idevflow/product-memory.json"), JSON.stringify(updated));
      },
    });
    assert.equal((await new RuntimeStore(repository).status())?.lifecycle, "testflight_handoff");
