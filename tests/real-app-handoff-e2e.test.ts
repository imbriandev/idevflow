...
import { recordReview } from "../extensions/pi-ios/lifecycle/service.ts";
...
import { acquireSimulatorLease, captureSimulatorScreenshot, releaseSimulatorLease } from "../extensions/pi-ios/simulator/service.ts";
...
describe("real app manual TestFlight handoff", () => {
  it("takes SampleApp through real release verification, local promotion, and a no-upload handoff", { skip: !enabled }, async () => {
    const root = await mkdtemp(join(tmpdir(), "pi-ios-real-handoff-")); roots.push(root, `${root}.pi-ios-promotion`);
    ...
    await execFileAsync("git", ["checkout", "-b", "pi-ios/integration"], { cwd: root });
    await mkdir(join(root, "docs", "pi-ios"), { recursive: true });
    await writeFile(join(root, "docs", "pi-ios", "privacy-review.json"), JSON.stringify({ schemaVersion: 1, decision: "go", dataPractices: [], permissions: [], findings: [] }));
    await writeFile(join(root, "docs", "pi-ios", "release.json"), JSON.stringify({ schemaVersion: 1, version: "1.0", build: "1", bundleId: "dev.piios.SampleApp", target: "testflight-internal", releaseNotes: "Verified SampleApp cutover", knownIssues: [], supportUrl: "https://example.com/support", privacyUrl: "https://example.com/privacy" }));
    ...
    await writeFile(join(root, ".pi-ios", "config.json"), JSON.stringify({ ...initialConfig, quality: { ...initialConfig.quality, performanceBudgets: { "Duration (AppLaunch)": 10 } } }));
    ...
    const evidenceDirectory = join(root, ".pi-ios", "evidence", session.id); await mkdir(evidenceDirectory, { recursive: true });
