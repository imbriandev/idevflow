...
import { acquireSimulatorLease, captureSimulatorScreenshot, releaseSimulatorLease } from "../extensions/appforge/simulator/service.ts";
...
describe("real app manual TestFlight handoff", () => {
  it("takes SampleApp through real release verification, local promotion, and a no-upload handoff", { skip: !enabled }, async () => {
    ...
    const evidenceDirectory = join(root, ".appforge", "evidence", session.id); await mkdir(evidenceDirectory, { recursive: true });
