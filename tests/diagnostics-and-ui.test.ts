...
describe("production diagnostics and interaction gates", () => {
  it("emits metadata-only diagnostics without exposing writer task content", async () => {
    ...
    const session: WriterSession = { id: randomUUID(), piSessionId: "diagnostic", stage: "build", task: secretTask, risk: "low", status: "stale", branch: "pi-ios/diagnostic", worktreePath: fixture.root, baseCommit: repository.head!, claims: ["README.md"], createdAt: now, heartbeatAt: now, leaseExpiresAt: now, statusReason: "test" };
  ...
  it("fails approval-required release and pipeline actions closed without an interactive UI", async () => {
    ...
    await assert.rejects(registry.tools.get("pi_ios_pipeline")!.execute("id", { action: "approve_risk", pipelineId: "demo", sliceId: "high-risk" }, undefined, undefined, context), /fails closed without interactive UI/);
    await assert.rejects(registry.tools.get("pi_ios_release")!.execute("id", { action: "approve" }, undefined, undefined, context), /fails closed without interactive UI/);
  });
  ...
  it("honors an interactive cancellation before coordinator mutation", async () => {
    ...
    const result: any = await registry.tools.get("pi_ios_pipeline")!.execute("id", { action: "cancel", pipelineId: "does-not-need-to-exist", reason: "test cancellation" }, undefined, undefined, rejectingInteractiveContext(fixture.root));
