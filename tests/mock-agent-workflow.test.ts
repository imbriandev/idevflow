...
afterEach(async () => { delete process.env.PI_IOS_WORKER_PACKET; for (const cleanup of cleanups.splice(0).reverse()) await cleanup(); });
    ...
    const evaluations = [
      {
        ...
        run: async () => { process.env.PI_IOS_WORKER_PACKET = "/packet.json"; return tools.get("pi_ios_pipeline")!.execute("call", { action: "status" }, undefined, undefined, context(fixture.root, true)); },
        error: /workers cannot invoke coordinator/,
      },
      {
        name: "untrusted mock agent cannot create a release candidate",
        run: async () => tools.get("pi_ios_release")!.execute("call", { action: "create_candidate", verificationFingerprint: "receipt" }, undefined, undefined, context(fixture.root, false)),
