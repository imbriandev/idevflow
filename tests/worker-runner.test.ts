...
import { PiWorkerLauncher } from "../extensions/idevflow/workers/runner.ts";
...
describe("isolated worker process", () => {
  it("spawns a separate supervised process and redacts its capability", async () => {
    const root = await mkdtemp(join(tmpdir(), "idev-worker-runner-")); roots.push(root);
    ...
    await writeFile(script, "console.log(`pid=${process.pid} capability=${process.env.IDEVFLOW_WORKER_CAPABILITY}`);\n");
    ...
    assert.equal(piArgs.some((arg) => arg.includes("idev_context")), true);
    assert.equal(piArgs.at(-1)?.includes("Call idev_context"), true);
  });
});
