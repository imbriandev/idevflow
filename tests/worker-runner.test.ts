...
import { PiWorkerLauncher } from "../extensions/canopy/workers/runner.ts";
...
describe("isolated worker process", () => {
  it("spawns a separate supervised process and redacts its capability", async () => {
    const root = await mkdtemp(join(tmpdir(), "canopy-worker-runner-")); roots.push(root);
    ...
    await writeFile(script, "console.log(`pid=${process.pid} capability=${process.env.CANOPY_WORKER_CAPABILITY}`);\n");
    ...
    assert.equal(piArgs.some((arg) => arg.includes("canopy_context")), true);
    assert.equal(piArgs.at(-1)?.includes("Call canopy_context"), true);
  });
});
