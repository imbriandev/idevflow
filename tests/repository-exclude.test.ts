...
describe("runtime Git exclusion", () => {
  it("keeps local runtime state out of source status without editing tracked ignore files", async () => {
    ...
    await execFileAsync("git", ["-c", "user.name=iDevFlow Tests", "-c", "user.email=tests@example.invalid", "commit", "-m", "remove ignore"], { cwd: fixture.root });
    ...
    assert.match(await readFile(join(repository.commonGitDirectory, "info", "exclude"), "utf8"), /^\.idevflow\/$/m);
  });
});
