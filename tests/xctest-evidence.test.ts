...
import { assertPassedXCTest, assertPerformanceBudget, assertQualityTestSource, validateXCTestMetadata } from "../extensions/idevflow/verification/xctest-evidence.ts";
...
describe("XCTest-backed quality evidence", () => {
  ...
  it("requires the named source test to contain the matching XCTest API", async () => {
    const root = await mkdtemp(join(tmpdir(), "idev-xctest-evidence-")); roots.push(root);
