...
import { loadContextReceipt } from "../extensions/canopy/context/receipts.ts";
...
import { registerContextTool } from "../extensions/canopy/tools/context-tool.ts";
...
describe("specialist context tool", () => {
  it("registers a read-only bounded selector with readable package paths", async () => {
    ...
    assert.equal(tool.name, "canopy_context");
  ...
  it("records a high-risk selection against the active writer session", async () => {
    const fixture = await createGitFixture(); cleanups.push(fixture.cleanup, async () => rm(`${fixture.root}.canopy-worktrees`, { recursive: true, force: true }));
