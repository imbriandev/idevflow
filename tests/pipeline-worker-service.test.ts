...
import { PIPELINE_SCHEMA_VERSION, type PipelineSliceState, type WorkerRunRecord } from "../extensions/appforge/pipeline/types.ts";
...
import { reportWorkerRepair, submitPipelineWorker } from "../extensions/appforge/workers/service.ts";
...
async function setup(id: string) {
  ...
  const packetPath = join(fixture.root, ".appforge", "pipeline", "packets", `${packetId}.json`); await mkdir(join(fixture.root, ".appforge", "pipeline", "packets"), { recursive: true }); const digest = await writeWorkerPacket(packetPath, packet);
