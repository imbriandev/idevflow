...
import { KNOWLEDGE_REFERENCES, detectKnowledgeSurfaces, selectKnowledge } from "../extensions/idevflow/context/knowledge.ts";
...
describe("specialist knowledge routing", () => {
  ...
  it("keeps skills and the package-owned specialist knowledge base aligned", async () => {
    ...
    for (const stage of ["define", "plan", "build", "test", "review", "ship", "learn"]) {
      const skill = await readFile(join(root, "skills", `idev-${stage}`, "SKILL.md"), "utf8");
      assert.match(skill, /idev_context/);
    }
  });
});
