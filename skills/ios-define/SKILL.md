...
9. Through write preflight, create the schema-version-3 configured `productMemory` and schema-version-2 `slcSpec` documents (defaults: `docs/pi-ios/product-memory.json` and `docs/pi-ios/slc.json`).
...
11. Run docs verification, postflight, finish, then `pi_ios_lifecycle integrate`. Use lifecycle status to report the validated product fingerprint. Recommend `/ios:plan` only after lifecycle reaches `defined`.
