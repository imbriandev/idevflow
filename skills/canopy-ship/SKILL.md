...
compatibility: Canopy; macOS/Xcode; iOS/macOS 26+, Swift 6.2+
---
...
3. Record a critical `stage=ship` context receipt with `canopy_context`. Prepare source-bound screenshot variants with `canopy_simulator`; prepare accessibility/performance inputs with `canopy_proof`, including the exact XCTest test identifiers and performance metric names. `canopy_verify` parses the fresh release xcresult, requires a passing `performAccessibilityAudit` test, and checks metric measurements against project-owned `quality.performanceBudgets`—metadata alone is not proof.
...
5. Call `canopy_release create_candidate` with the fresh release fingerprint. Present the exact candidate, target, gates, and known issues.
6. Call `canopy_release approve` only when the founder is ready; use its expiring single-use token for exact local `promote`.
