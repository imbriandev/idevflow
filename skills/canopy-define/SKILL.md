...
9. Through write preflight, create the schema-version-3 configured `productMemory` and schema-version-2 `slcSpec` documents (defaults: `docs/canopy/product-memory.json` and `docs/canopy/slc.json`).
...
11. Run docs verification, postflight, finish, then `canopy_lifecycle integrate`. Use lifecycle status to report the validated product fingerprint. Recommend `/canopy:plan` only after lifecycle reaches `defined`.

## Specialist context

For product ambiguity, onboarding, interface language, accessibility wording, or a paid-value hypothesis, call `canopy_context` with `stage=define`, risk, task, and relevant surfaces. Read only its returned package paths. Apply `product-interface.md`; add `monetization.md` only when paid behavior is in scope. Do not load implementation checklists during ordinary product definition.
